        ...
        <div class="box-picture">
            ...
            <?php if ($totalPage > $limitPage && $paginate == true) : ?>
                <nav class="box-link-page">
                    <a href="<?= $this->route('image/pag/1') ?>" class="link-all link-blue">Mostrar mas</a>
                </nav>
            <?php endif; ?>
            <?php if ($totalPage > $limitPage && $paginate == false) : ?>
                <nav class="box-link-page">
                    <a href="<?= $this->route('image/pag/1') ?>" class="link-page link-light"> &lt;&lt; </a>
                    <?php if ($pagActual > 1) : ?>
                        <a href="<?= $this->route('image/pag/' . ($pagActual - 1)) ?>" class="link-page link-light"> &lt; </a>
                    <?php else : ?>
                    ...
                    <?php for ($i = 0; $i < $pagLink; $i++) : ?>
                        ...
                        <?php else : ?>
                            <a href="<?= $this->route('image/pag/' . ($i + 1)) ?>" class="link-page link-light"><?= ($i + 1) ?></a>
                        <?php endif; ?>
                    <?php endfor; ?>
                    ...
                    <?php else : ?>
                        <span class="link-page link-disabled"> &gt; </span>
                    <?php endif; ?>
                    <a href="<?= $this->route('image/pag/' . $pagLink) ?>" class="link-page link-light"> &gt;&gt; </a>
                </nav>
            <?php endif; ?>
...
<script src="<?php $this->assets("src/js/image.js"); ?>"></script>
